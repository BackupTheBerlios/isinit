...
<xsl:stylesheet version="1.1"
  ...
  <xsl:variable name="cheminGeneration" select="document('tmp.xmi')//iepp:processus">
      <xsl:value-of select="@cheminGeneration"/>
  </xsl:variable>
  <xsl:variable name="fileSeparator" select="system-property('file.separator')"/>

  <xsl:template name="supprimeCheminRelatif">
    <xsl:param name="chaine"/>
    <xsl:choose>
      <xsl:when test="contains($chaine,'../')">
        <xsl:variable name="tmp">
          <xsl:value-of select="substring($chaine,4)"/>
        </xsl:variable>
        <xsl:call-template name="supprimeCheminRelatif">
          <xsl:with-param name="chaine" select="$tmp"/>
        </xsl:call-template>
      </xsl:when>
      <xsl:otherwise>
  ...
  <xsl:template name="getCheminComposant">
    <xsl:param name="chaine"/>
  ...
  <xsl:template name="process-style">
    ...
    <xsl:if test="$name">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
            <!-- Suppresion des éléments bloquants -->
          <!--<xsl:attribute name="{$name}">
            <xsl:value-of select="$value"/>
          </xsl:attribute>-->
        </xsl:otherwise>
  ...
  <xsl:template name="process-table">
   ...
   <xsl:attribute name="width">16cm</xsl:attribute>
    ...
    <!-- inline-progression-dimension non suporté par fop -->
    <!--<xsl:if test="@width">
    ...
    <xsl:variable name="largeurColonne">
      <xsl:variable name="nbColonne">
        <xsl:value-of select="count(tbody/tr[1]/td | tbody/tr[1]/th)"/>
      </xsl:variable>
      <xsl:value-of select="16 div $nbColonne"/>
    </xsl:variable>

    <xsl:choose>
      <xsl:when test="tbody">
        <xsl:for-each select="tbody/tr[1]/td | tbody/tr[1]/th">
          <xsl:element name="fo:table-column">
            <xsl:attribute name="column-width">
              <xsl:value-of select="concat($largeurColonne,'cm')"/>
            </xsl:attribute>
          </xsl:element>
  ...
  <xsl:template name="process-img">
    <xsl:attribute name="src">
      ...
      <xsl:choose>
        <xsl:when test="$cheminGeneration">

          <xsl:variable name="cheminComposant">
            <xsl:call-template name="getCheminComposant">
              <xsl:with-param name="chaine">
                <xsl:value-of select="//span[@class='cheminRelatif']/@id"/>
              </xsl:with-param>
            </xsl:call-template>
          </xsl:variable>
          ...
          <xsl:value-of select="concat($cheminGeneration,$fileSeparator,$cheminComposant,$fileSeparator,$image)"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="@src"/>
        </xsl:otherwise>
